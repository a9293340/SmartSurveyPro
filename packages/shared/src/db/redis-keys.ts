...
export class RedisKeyBuilder {
  ...
  // 靜態方法：常用 key 建構器
  static web() { return new RedisKeyBuilder('Web') }
  static server() { return new RedisKeyBuilder('Server') }
  static auth() { return new RedisKeyBuilder('Auth') }
  static survey() { return new RedisKeyBuilder('Survey') }
  static analytics() { return new RedisKeyBuilder('Analytics') }
}
...
export const RedisKeys = {
  ...
  Auth: {
    userSession: (userId: string) =>
      RedisKeyBuilder.auth().feature('session').id(userId).build(),
    refreshToken: (tokenId: string) =>
      RedisKeyBuilder.auth().feature('refresh').id(tokenId).build(),
    loginAttempts: (ip: string) =>
      RedisKeyBuilder.auth().feature('attempts').id(ip).build(),
  },

  // Web 應用快取
  Web: {
    surveyCache: (surveyId: string) =>
      RedisKeyBuilder.web().feature('survey').id(surveyId).build(),
    userPreferences: (userId: string) =>
      RedisKeyBuilder.web().feature('prefs').id(userId).build(),
  ...
  // 後端服務
  Server: {
    taskQueue: (taskType: string) =>
      RedisKeyBuilder.server().feature('queue').id(taskType).build(),
    ...
    rateLimitIP: (ip: string) =>
      RedisKeyBuilder.server().feature('rate').id(`ip:${ip}`).build(),
  },
  ...
  Analytics: {
    dailyStats: (date: string) =>
      RedisKeyBuilder.analytics().feature('daily').id(date).build(),
