...
### 來自 Task 4.2.1 - 前端驗證邏輯 ✅

#### 🔧 條件式驗證規則

- **檔案位置**：`apps/web/app/composables/useFormValidation.ts:5`
- **當前狀態**：基礎規則型驗證引擎已完成，支援固定驗證規則
- **延後原因**：Phase 1 專注基礎驗證功能，動態條件留待邏輯跳轉功能
- **預計實作**：Phase 2 邏輯跳轉引擎開發時
- **影響程度**：中 - 提升問卷智能化和用戶體驗
- **實作內容**：
  ```typescript
...
- **檔案位置**：`apps/web/app/composables/useFormValidation.ts:6`
- **當前狀態**：內建常用驗證規則，架構支援擴展
- **延後原因**：Phase 1 內建規則足夠 MVP 需求
- **預計實作**：Phase 2 企業客戶需求階段
- **影響程度**：中 - 提升系統彈性和客製化能力
- **實作內容**：
  ```typescript
  // TODO(feature): 自訂驗證規則 - 允許動態注入自訂驗證邏輯 (Phase 2)
  // 升級計劃：
  -驗證規則註冊系統 - 安全的規則執行沙盒 - 規則庫管理介面 - 預設規則模板庫;
  ```

#### 🔗 跨欄位驗證

- **檔案位置**：`apps/web/app/composables/useFormValidation.ts:7`
- **當前狀態**：僅支援單一欄位驗證
- **延後原因**：Phase 1 專注基礎功能，跨欄位邏輯複雜度高
- **預計實作**：Phase 2 進階驗證功能階段
- **影響程度**：中 - 支援複雜表單驗證場景
- **實作內容**：
  ```typescript
  // TODO(feature): 跨欄位驗證 - 支援欄位間的相依性驗證 (Phase 2)
  // 升級計劃：
  -欄位依賴關係圖 - 批次驗證協調器 - 循環依賴檢測 - 驗證順序優化;
  ```

#### ⚡ 異步驗證

- **檔案位置**：`apps/web/app/composables/useFormValidation.ts:8`
- **當前狀態**：僅支援同步驗證規則
- **延後原因**：Phase 3 才需要複雜的服務端驗證整合
- **預計實作**：Phase 3 企業級功能階段
- **影響程度**：低 - 進階功能，不影響基礎使用
- **實作內容**：
  ```typescript
  // TODO(feature): 異步驗證 - 支援需要 API 檢查的驗證規則 (Phase 3)
  // 升級計劃：
  -異步驗證隊列管理 - 結果快取機制 - 請求去重和防抖 - 驗證狀態可視化指示;
  ```

### 來自 Task 4.1.2 - 進度顯示組件 🔄

#### ⚡ 進度計算效能優化

- **檔案位置**：`apps/web/app/components/response/ProgressIndicator.vue`
- **當前狀態**：每次狀態變更都重新計算所有進度數據
- **延後原因**：Phase 1 專注功能實現，效能優化留待後續
- **預計實作**：Phase 2 效能優化階段
- **影響程度**：低 - 對小型問卷（<50題）影響不大
- **實作內容**：
  ```typescript
  // TODO(feature): 進度計算效能優化 - Phase 2 效能優化階段
  // 升級計劃：
  - 進度數據緩存機制
  - 增量計算策略
  - Web Worker 背景計算
...
- **檔案位置**：`apps/web/app/components/response/QuestionProgress.vue`
- **當前狀態**：僅顯示基本完成狀態統計
- **延後原因**：需要更複雜的用戶行為分析，Phase 1 專注基礎體驗
- **預計實作**：Phase 2 分析增強階段
- **影響程度**：低 - 屬於進階功能，不影響基本填寫流程
- **實作內容**：
  ```typescript
  // TODO(feature): 進階統計功能 - Phase 2 分析增強階段
  // 升級計劃：
  - 題目平均作答時間分析
  - 困難題目識別（長時間停留）
  - 跳過率統計和警告
...
- **檔案位置**：`apps/web/app/components/response/` 全部進度組件
- **當前狀態**：使用固定的 Tailwind 樣式，無客製化支援
- **延後原因**：主題系統為 Phase 2 規劃功能
- **預計實作**：Phase 2 主題系統階段
- **影響程度**：中 - 影響企業客戶品牌一致性需求
- **實作內容**：
  ```typescript
  // TODO(feature): 主題客製化支援 - Phase 2 主題系統階段
  // 升級計劃：
  - 進度條顏色自訂
  - 圖示和字體主題化
  - 企業 Logo 整合
...
- **檔案位置**：`apps/web/app/components/response/QuestionProgress.vue`
- **當前狀態**：基本響應式支援，複雜互動在小屏幕體驗待優化
- **延後原因**：Phase 1 專注桌面端體驗，移動端深度優化留待後續
- **預計實作**：Phase 2 移動端優化階段
- **影響程度**：中 - 影響移動端用戶體驗
- **實作內容**：
  ```typescript
  // TODO(feature): 進階響應式最佳化 - Phase 2 移動端優化階段
  // 升級計劃：
  -滑動手勢導航 - 觸控友好的交互元素 - 動態字體大小調整 - 一手操作模式支援;
  ```
