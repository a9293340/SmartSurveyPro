...
async function archiveOldResponses() {
  ...
  // 1. 查找需要歸檔的資料
  const toArchive = await db.responses.find({
...
```javascript
// 使用投影減少傳輸資料
const surveyList = await db.surveys.find(
  { created_by: userId },
  { 
    projection: {
      title: 1,
...
*文件版本：v1.0 | 最後更新：2025-01-10*
*適用於：SmartSurvey MongoDB Atlas M5 (無分片) + Redis Cloud 架構*